...
class IonicNativeAuthPlugin: CDVPlugin {
    ...
    private func getVaultFromStore(_ key: String) -> IdentityVault? {
        for vault in vaultStore! where vault.descriptor.key == key {
            return vault
        }
        return nil
    }
    ...
    private func getDescriptorFromCommand(_ command: CDVInvokedUrlCommand) throws -> VaultDescriptor {
        ...
        guard let username = descriptors["username"] as? String else {
            throw VaultError.invalidArguments("missing username")
        }
        guard let vaultId = descriptors["vaultId"] as? String else {
            throw VaultError.invalidArguments("missing vaultId")
        }
